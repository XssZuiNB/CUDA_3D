...
template <typename T> class NISymmetricEigensolver3x3
{
public:
    ...
    void operator()(T a00, T a01, T a02, T a11, T a12, T a22, int32_t sortType,
...
private:
    ...
    {
        std::array<T, 3> cross = {U[1] * V[2] - U[2] * V[1], U[2] * V[0] - U[0] * V[2],
                                  U[0] * V[1] - U[1] * V[0]};
        return cross;
    }

    void ComputeOrthogonalComplement(std::array<T, 3> const &W, std::array<T, 3> &U,
    ...
    void ComputeEigenvector0(T a00, T a01, T a02, T a11, T a12, T a22, T eval0,
                             std::array<T, 3> &evec0) const
    {
                             ...
                             std::array<T, 3> const &evec0, T eval1, std::array<T, 3> &evec1) const
    {
        ...
        std::array<T, 3> AU = {a00 * U[0] + a01 * U[1] + a02 * U[2],
                               a01 * U[0] + a11 * U[1] + a12 * U[2],
                               a02 * U[0] + a12 * U[1] + a22 * U[2]};

        std::array<T, 3> AV = {a00 * V[0] + a01 * V[1] + a02 * V[2],
