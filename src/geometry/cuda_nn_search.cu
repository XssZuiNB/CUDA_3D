        ...
        for (auto i = ix_begin; i < ix_end; i++)
        {
            for (auto j = iy_begin; j < iy_end; j++)
            {
                for (auto k = iz_begin; k < iz_end; k++)
                {
                    ...
                    auto neighbor_grid_cell = m_grid_cells_R_ptr[idx_neighbor_grid_cell];
                    ...
                    for (auto offset = 0; offset < n_points; offset++)
                    {
                        ...
                        auto idx_of_neighbor_point =
                            m_points_sorted_idxs_R_ptr[idx_of_sorted_points_indecies];
                        auto neighbor_point = m_points_R_ptr[idx_of_neighbor_point];
                        ...
                        if (euclidean_distance_square < distance_square)
                        {
                            distance_square = euclidean_distance_square;
    ...
    const float search_radius)
{
    ...
    thrust::device_vector<gca::index_t> idxs_of_sorted_idxs_R(n_points_R);
    thrust::sequence(idxs_of_sorted_idxs_R.begin(), idxs_of_sorted_idxs_R.end());
    err = cudaGetLastError();
    if (err != ::cudaSuccess)
    {
        return err;
    ...
    auto end_iter_idxs_of_sorted_idxs_R =
        thrust::reduce_by_key(grid_cell_idxs_R.begin(), grid_cell_idxs_R.end(),
                              idxs_of_sorted_idxs_R.begin(), thrust::make_discard_iterator(),
                              idxs_of_sorted_idxs_R.begin(), thrust::equal_to<gca::index_t>(),
    ...
    if (unique_grid_cells_n_R != end_iter_idxs_of_sorted_idxs_R - idxs_of_sorted_idxs_R.begin())
    {
        return ::cudaErrorInvalidValue;
    }
    ...
    thrust::for_each(thrust::make_zip_iterator(
                         thrust::make_tuple(grid_cell_idxs_R.begin(), idxs_of_sorted_idxs_R.begin(),
                                            points_counter_per_grid_cell_R.begin())),
                     thrust::make_zip_iterator(thrust::make_tuple(
                         grid_cell_idxs_R.begin() + unique_grid_cells_n_R,
                         idxs_of_sorted_idxs_R.begin() + unique_grid_cells_n_R,
