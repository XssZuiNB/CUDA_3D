...
struct add_and_count_points_functor
{
    ...
    __forceinline__ __device__ thrust::tuple<gca::point_t, float3, gca::counter_t> operator()(
        const thrust::tuple<gca::point_t, float3, gca::counter_t> &fir,
        const thrust::tuple<gca::point_t, float3, gca::counter_t> &sec)
    {
        auto &fir_pts = thrust::get<0>(fir);
        auto &sec_pts = thrust::get<0>(sec);
        auto &fir_normal = thrust::get<1>(fir);
        auto &sec_normal = thrust::get<1>(sec);
        return thrust::make_tuple(
            gca::point_t{fir_pts.coordinates + sec_pts.coordinates, fir_pts.color + sec_pts.color,
                         gca::point_property::inactive},
            fir_normal + sec_normal, thrust::get<2>(fir) + thrust::get<2>(sec));
    }
};
...
struct compute_points_mean_functor
{
    ...
    __forceinline__ __device__ thrust::tuple<gca::point_t, float3> operator()(
        const thrust::tuple<gca::point_t, float3> &points_normal_sum, const gca::counter_t n)
    {
        auto &points_sum = thrust::get<0>(points_normal_sum);
        auto &normal_sum = thrust::get<1>(points_normal_sum);
        return thrust::make_tuple(gca::point_t{points_sum.coordinates / n, points_sum.color / n,
                                               gca::point_property::inactive},
                                  normal_sum / n);
    }
};
...
/*
void cuda_voxel_grid_downsample(thrust::device_vector<gca::point_t> &pts,
                                thrust::device_vector<float3> &normals,
                                const thrust::device_vector<gca::point_t> &src_points,
                                const thrust::device_vector<float3> &src_normals,
                                const float3 &voxel_grid_min_bound, const float voxel_size)
{
    ...
    auto pt_normal_zipped_iter_begin =
        thrust::make_zip_iterator(thrust::make_tuple(src_points.begin(), src_normals.begin()));
    auto get_point_with_sorted_index_iter =
        thrust::make_permutation_iterator(pt_normal_zipped_iter_begin, index_vec.begin());
